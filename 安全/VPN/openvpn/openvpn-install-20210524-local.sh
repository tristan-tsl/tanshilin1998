...
function installUnbound() {
	...
	if [[ ! -e /etc/unbound/unbound.conf ]]; then
		...
		if [[ $OS =~ (debian|ubuntu) ]]; then
			...
			# Configuration
			echo 'interface: 10.8.2.1
access-control: 10.8.2.1/24 allow
hide-identity: yes
hide-version: yes
use-caps-for-id: yes
prefetch: yes' >>/etc/unbound/unbound.conf
			...
			# Configuration
			sed -i 's|# interface: 0.0.0.0$|interface: 10.8.2.1|' /etc/unbound/unbound.conf
			sed -i 's|# access-control: 127.0.0.0/8 allow|access-control: 10.8.2.1/24 allow|' /etc/unbound/unbound.conf
...
ifconfig-pool-persist ipp.txt" >>/etc/openvpn/server.conf
	...
	2) # Self-hosted DNS resolver (Unbound)
		echo 'push "dhcp-option DNS 10.8.2.1"' >>/etc/openvpn/server.conf
		if [[ $IPV6_SUPPORT == 'y' ]]; then
